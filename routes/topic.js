    ...
    .then(topics => {
      ...
      .then(topic => {
        if(topic.length == 0) {
          ...
          res.render('topic/sampleDel', {topics: topics, topic: topic[0]});
        }
      })
      .catch(err => {
        console.log(err);
...
router.get(['/:id/edit'], (req, res, next) => {
  db.select('id', 'title').from('topic')
    .then(topics => {
      ...
      if(id) {
        db('topic').whereRaw('id=?', [id])
          .then(topic => { res.render('topic/sampleEdit',
        ...
        res.render('topic/sampleList', {topics: topics});
      }
    })
    .catch(err => {
...
router.get('/add', (req, res, next) => {
  db.select('id', 'title').from('topic')
    .then(rows => { res.render('topic/sampleAdd', {topics: rows}); })
    .catch(err => {
      console.log(err);
      res.status(500).send('Internal Server Error');
    });
  ...
  db.select('id', 'title').from('topic')
    .then( rows => {
      ...
      if(id) {
        db('topic').whereRaw('id=?', [id])
          .then(result => {
            res.render('topic/sampleList', {topics: rows, topic: result[0]});
          })
          .catch(err => {
            console.log(err);
            res.status(500).send('Internal Server Error');
        ...
        res.render('topic/sampleList', {topics: rows});
      }
    })
    .catch(err => {
      console.log(err);
